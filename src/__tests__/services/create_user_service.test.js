...
import { processingUsers, validationUserErrors, addedUser } from '../../actions/users'
...
describe('Create User Service', () => {
  ...
  describe('with invalid attributes', () => {
    ...
    it("should be dispatch processing and validation errors actions", async () => {
      ...
      expect(actions).toHaveLength(2);
      expect(actions[0]).toEqual(processingUsers());
      expect(actions[1]).toEqual(validationUserErrors({ username: ['обязательно для заполнения'] }));
    });
