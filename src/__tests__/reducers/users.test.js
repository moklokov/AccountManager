...
describe("Users reducer", () => {
  describe("initialization", () => {
    ...
    describe("with store and empty action", () => {
      let state = {
        items: { 1: { id: 1, first_name: "Test", last_name: "Testov" } },
        isProcessing: false,
        error: "",
        validErrors: {}
      };
  ...
  describe("processing", () => {
    ...
    it("should be changed processing state and clear errors", () => {
      expect(users(state, UsersActions.processingUsers())).toEqual(
        expectedState
      );
    });
  });
  ...
  describe("process error", () => {
    ...
    it("should be changed processing and error", () => {
      expect(users(state, UsersActions.processUsersError(error))).toEqual(
        expectedState
      );
    });
  });
  ...
  describe("validation errors`", () => {
    ...
    it("should be changed processing and errors", () => {
      expect(users(state, UsersActions.validationUserErrors(errors))).toEqual(
        expectedState
      );
    });
  });
  ...
  describe("loaded", () => {
    const user = { id: 1, first_name: "Test", last_name: "Testov" };
    ...
    it("should be changed users and processing state", () => {
      expect(users(state, UsersActions.loadedUsers(loadedUsers))).toEqual(
        expectedState
      );
    });
  });
  ...
  describe("edited", () => {
    ...
    const expectedState = {
      items: { 1: { id: 1, first_name: "New", last_name: "Testov" } },
      isProcessing: false,
      error: "",
      validErrors: {}
    };
    ...
    it("should be changed user and processing state", () => {
      expect(
        users(state, UsersActions.editedUser(1, { first_name: "New" }))
