...
export default function(state = initState(), action = {}) {
  switch (action.type) {
    ...
    case TYPES.USERS_LOADED:
      const users = action.users.reduce((users, user) => {
        users[user.id] = user;
        return users;
      }, {});
      return { ...state, items: users, isProcessing: false };
    case TYPES.USER_ADDED:
      return {
        ...state,
        items: { [action.user.id]: action.user, ...state.items },
        isProcessing: false
      };
    case TYPES.USER_EDITED:
      ...
      return {
        ...state,
        items: { ...state.items, [action.id]: updatedUser },
        isProcessing: false
      };
